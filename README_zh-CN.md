...
由于平台原因，如果你需要将 kemono.cr 的收藏（喜欢的帖子和画师）迁移到 `pawchive.st`，可以使用附带的迁移脚本：

1. **导出 kemono.cr 收藏**
   在 `.env` 中配置 `KEMONO_COOKIE_HEADER`（从浏览器 F12 网络面板复制包含 `__ddg*` 和 `session` 等的完整 Cookie 头）。
   然后运行：
   ```bash
   python export_kemono_favorites.py
   ```
   这会导出 `kemono_favorites_posts.json` 和 `kemono_favorites_artists.json`。

2. **导入到 pawchive.st**
   在 `.env` 中配置 `PAWCHIVE_COOKIE_HEADER`（同样从浏览器复制 pawchive.st 的 Cookie 头）。
   然后运行：
   ```bash
